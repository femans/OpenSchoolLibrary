	...
	try {
		...
		for (let i = 1; i < lines.length; i++) {
			try {
				...
				if (copyCount > 0 && book) {
					const copies = Array(copyCount).fill(null).map(() => ({
						org_id: orgId,
						book_id: book.id,
						status: 'available' as const
					}));

					const { error: copiesError } = await supabaseServer
						.from('copies')
						.insert(copies);

					if (copiesError) {
						results.errors.push(`Line ${i + 1}: Book created but copies failed: ${copiesError.message}`);
					}
				}
